...
class RedObstaclePerception:
    def __init__(self):
        ...
        # Fixed manoeuvre: stop briefly, then follow a slow circular arc.
        self.stop_hold_seconds = float(rospy.get_param("~stop_hold_seconds", 0.40))
        self.dodge_seconds = float(rospy.get_param("~dodge_seconds", 3.5))
        self.cooldown_seconds = float(rospy.get_param("~cooldown_seconds", 2.0))
        self.dodge_linear_speed = float(rospy.get_param("~dodge_linear_speed", 0.12))
        self.dodge_angular_speed = float(rospy.get_param("~dodge_angular_speed", 0.40))
        ...
        rospy.loginfo(
            "Dodge: stop=%.2fs arc=%.2fs v=%.2f w=%.2f cooldown=%.2fs",
            self.stop_hold_seconds, self.dodge_seconds, self.dodge_linear_speed,
            self.dodge_angular_speed, self.cooldown_seconds,
        )
        rospy.loginfo(
    ...
    def start_fixed_dodge(self, turn_direction):
        ...
        self.controller_state_started = rospy.Time.now()
        rospy.logwarn("[overwatch] CLOSE OBSTACLE: stop, then fixed dodge %s", turn_direction)
    ...
    def control_timer_callback(self, _event):
        ...
        if self.controller_state == "GLOBAL":
            ...
            if elapsed >= self.stop_hold_seconds:
                self.controller_state = "DODGE_ARC"
                self.controller_state_started = now

        elif self.controller_state == "DODGE_ARC":
            command.linear.x = max(0.0, self.dodge_linear_speed)
            sign = 1.0 if self.committed_turn_direction == "LEFT" else -1.0
            command.angular.z = sign * abs(self.dodge_angular_speed)
            if elapsed >= self.dodge_seconds:
                self.controller_state = "COOLDOWN"
                self.controller_state_started = now
                command = self.zero_twist()

        elif self.controller_state == "COOLDOWN":
            # Global navigation is restored immediately, but obstacle triggers
            # are ignored briefly so the same tree cannot retrigger the dodge.
            command = self.fresh_global_cmd()
    ...
    def image_callback(self, message):
        ...
        if controller_state.startswith("SHARP TURN") and self.controller_state == "GLOBAL":
