#!/usr/bin/env python3
"""
red_obstacle_fixed_dodge_controller.py
...
The node relays the global velocity command normally. When a close obstacle is
confirmed, it stops briefly, performs one fixed circular dodge, then returns
control to the global command source.
"""
...
class RedObstaclePerception:
    def __init__(self):
        ...
        self.target_min_bottom = float(
            rospy.get_param("~target_min_bottom", 0.50)
        )
        ...
        # ==================================================
        # Fixed-dodge controller
        # ==================================================
        # IMPORTANT: configure move_base/global controller to publish to
        # global_cmd_topic. This node publishes the final command to
        # output_cmd_topic and temporarily overrides the global command.
        ...
        # Fixed S-shaped bypass manoeuvre:
        #   1. stop briefly,
        #   2. turn away from the obstacle,
        #   3. drive forward past it,
        #   4. turn back by the same amount,
        #   5. return control to move_base.
        #
        # This is deliberately not one long circular arc. A single arc mostly
        # changes the robot heading; this sequence creates lateral clearance
        # around a tree and then approximately restores the original heading.
        self.stop_hold_seconds = float(
            rospy.get_param("~stop_hold_seconds", 0.40)
        )
        self.turn_away_seconds = float(
            rospy.get_param("~turn_away_seconds", 2.25)
        )
        self.pass_straight_seconds = float(
            rospy.get_param("~pass_straight_seconds", 4.0)
        )
        self.turn_back_seconds = float(
            rospy.get_param("~turn_back_seconds", 2.25)
        )
        self.cooldown_seconds = float(
            rospy.get_param("~cooldown_seconds", 2.0)
        )

        self.dodge_linear_speed = float(
            rospy.get_param("~dodge_linear_speed", 0.12)
        )
        self.dodge_angular_speed = float(
            rospy.get_param("~dodge_angular_speed", 0.35)
        )
        self.pass_straight_speed = float(
            rospy.get_param("~pass_straight_speed", 0.12)
        )

        self.controller_state = "GLOBAL"
        self.controller_state_started = rospy.Time.now()
        self.committed_turn_direction = None
        self.latest_global_cmd = Twist()
        self.latest_global_cmd_time = rospy.Time(0)

        # Do not allow the same still-visible obstacle to retrigger another
        # fixed dodge immediately after cooldown. Rearm only after the close
        # target has disappeared for several consecutive image frames.
        self.dodge_armed = True
        self.rearm_clear_frames = int(rospy.get_param("~rearm_clear_frames", 10))
        self.rearm_clear_count = 0
        ...
        rospy.Subscriber(
            self.global_cmd_topic, Twist, self.global_cmd_callback, queue_size=1
        )
        self.control_timer = rospy.Timer(
            rospy.Duration(1.0 / max(1.0, self.control_rate)),
        ...
        rospy.loginfo("[red_obstacle_perception] fixed-dodge controller running")
        ...
        rospy.loginfo("Global cmd: %s -> final cmd: %s", self.global_cmd_topic, self.output_cmd_topic)
        rospy.loginfo(
            "Dodge: stop=%.2fs away=%.2fs pass=%.2fs back=%.2fs "
            "v_arc=%.2f w=%.2f v_pass=%.2f cooldown=%.2fs",
            self.stop_hold_seconds,
            self.turn_away_seconds,
            self.pass_straight_seconds,
            self.turn_back_seconds,
            self.dodge_linear_speed,
            self.dodge_angular_speed,
            self.pass_straight_speed,
    ...
    def draw_controller_preview(self, debug, state, target_blob, dangers, x0, x1, y0, y1):
        ...
        cv2.putText(
            debug,
            "CONTROLLER: {} | DETECTOR: {}".format(self.controller_state, state),
    ...
    # ------------------------------------------------------------------
    # Actual fixed-dodge command arbitration
    # ------------------------------------------------------------------
    def global_cmd_callback(self, message):
        self.latest_global_cmd = message
        self.latest_global_cmd_time = rospy.Time.now()

    @staticmethod
    def zero_twist():
        return Twist()

    def fresh_global_cmd(self):
        age = (rospy.Time.now() - self.latest_global_cmd_time).to_sec()
        if self.latest_global_cmd_time == rospy.Time(0) or age > self.global_cmd_timeout:
            return self.zero_twist()
        return self.latest_global_cmd

    def start_fixed_dodge(self, turn_direction):
        if self.controller_state != "GLOBAL":
            return
        self.committed_turn_direction = turn_direction
        self.controller_state = "STOP_HOLD"
        self.controller_state_started = rospy.Time.now()
        rospy.logwarn(
            "[overwatch] CLOSE OBSTACLE: stop, bypass %s, pass, then return heading",
            turn_direction,
        )

    def control_timer_callback(self, _event):
        now = rospy.Time.now()
        elapsed = (now - self.controller_state_started).to_sec()
        command = Twist()

        # Positive angular.z is left; negative angular.z is right.
        away_sign = (
            1.0 if self.committed_turn_direction == "LEFT" else -1.0
        )

        if self.controller_state == "GLOBAL":
            command = self.fresh_global_cmd()

        elif self.controller_state == "STOP_HOLD":
            command = self.zero_twist()
            if elapsed >= self.stop_hold_seconds:
                self.controller_state = "TURN_AWAY"
                self.controller_state_started = now

        elif self.controller_state == "TURN_AWAY":
            command.linear.x = max(0.0, self.dodge_linear_speed)
            command.angular.z = away_sign * abs(self.dodge_angular_speed)

            if elapsed >= self.turn_away_seconds:
                self.controller_state = "PASS_STRAIGHT"
                self.controller_state_started = now

        elif self.controller_state == "PASS_STRAIGHT":
            command.linear.x = max(0.0, self.pass_straight_speed)
            command.angular.z = 0.0

            if elapsed >= self.pass_straight_seconds:
                self.controller_state = "TURN_BACK"
                self.controller_state_started = now

        elif self.controller_state == "TURN_BACK":
            command.linear.x = max(0.0, self.dodge_linear_speed)

            # Apply the opposite turn for the same duration so the final
            # heading is approximately the same as before the dodge.
            command.angular.z = -away_sign * abs(self.dodge_angular_speed)

            if elapsed >= self.turn_back_seconds:
                self.controller_state = "COOLDOWN"
                self.controller_state_started = now
                command = self.zero_twist()

        elif self.controller_state == "COOLDOWN":
            # Navigation commands are forwarded again, but the detector stays
            # disarmed until the handled obstacle has disappeared.
            command = self.fresh_global_cmd()
            ...
            rospy.logerr(
                "[overwatch] unknown controller state: %s",
    ...
    def image_callback(self, message):
        ...
        # Rearm only after the previously handled obstacle has disappeared.
        if target_blob is None:
            self.rearm_clear_count += 1
            if self.rearm_clear_count >= self.rearm_clear_frames:
                self.dodge_armed = True
        else:
            self.rearm_clear_count = 0
        ...
        if (
            self.dodge_armed
            and controller_state.startswith("SHARP TURN")
            and self.controller_state == "GLOBAL"
        ):
            direction = "LEFT" if controller_state.endswith("LEFT") else "RIGHT"
            self.dodge_armed = False
            self.start_fixed_dodge(direction)
        ...
        rospy.loginfo_throttle(
            1.0,
            "[perception] candidates=%d target=%s missed=%d max_danger=%.2f detector=%s controller=%s",
            len(blobs),
