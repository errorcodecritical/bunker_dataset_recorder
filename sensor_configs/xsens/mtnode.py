...
class XSensDriver(rclpy.node.Node):
    def __init__(self):
        ...
        self.sync_timestamps = {
            "ros": {
                "first": {"sec": 0, "nanosec": 0},
            },
            "hw": {
                "first": {"sec": 0, "nanosec": 0},
                "latest": {"sec": 0, "nanosec": 0},
            }
        }
    ...
    def spin_once(self):
        ...
        def fill_from_Timestamp(o):
            ...
            try:
                ...
                if not self.first_msg:
                    epoch_ts = self.get_clock().now()
                    
                    self.sync_timestamps['ros']['first']['sec'] = epoch_ts.nanoseconds // 1_000_000_000
                    self.sync_timestamps['ros']['first']['nanosec'] = epoch_ts.nanoseconds % 1_000_000_000
                    self.sync_timestamps['hw']['first']['sec'] = secs
                    self.sync_timestamps['hw']['first']['nanosec'] = int(nsecs)
                    
                    self.first_msg = True
                
                self.sync_timestamps['hw']['latest']['sec'] = secs
                self.sync_timestamps['hw']['latest']['nanosec'] = int(nsecs)
        ...
        #ISR MODIFICATIONS
        if self.first_msg and self.time_ref_pub:
            delta_sec = self.sync_timestamps['hw']['latest']['sec'] - self.sync_timestamps['hw']['first']['sec']
            delta_nanosec = self.sync_timestamps['hw']['latest']['nanosec'] - self.sync_timestamps['hw']['first']['nanosec']

            if delta_nanosec < 0:
                delta_sec -= 1
                delta_nanosec += 1_000_000_000
            
            synced_seconds = self.sync_timestamps['ros']['first']['sec'] + delta_sec
            synced_nanoseconds = self.sync_timestamps['ros']['first']['nanosec'] + delta_nanosec

            if synced_nanoseconds >= 1_000_000_000:
                synced_seconds += synced_nanoseconds // 1_000_000_000
                synced_nanoseconds = synced_nanoseconds % 1_000_000_000

            # self.get_logger().info(f"Synced timestamp: {synced_seconds}s and {synced_nanoseconds}ns (delta: {delta_sec}s and {delta_nanosec}ns)")

            self.h.stamp = rclpy.time.Time(
                seconds=synced_seconds,
                nanoseconds=synced_nanoseconds).to_msg()
